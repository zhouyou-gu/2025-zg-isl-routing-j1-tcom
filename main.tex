...
\blue{To tackle this mixed-integer problem, we apply the Lagrangian dual relaxation method by lifting the maximum rate limitation constraints between neighboring satellites via Lagrange multipliers.}
...
\blue{The converted throughput can also fluctuate across iterations, but it approaches a stable level as the multipliers settle.} Also, the network throughput is higher in $\beta=0.5$ than the ones in $\beta=0.7$ and $0.9$, indicating that a slower step size decay rate leads to better performance due to more exploration of the Lagrange multipliers' space.
...
\blue{It has also shown that the method can be practically computable for this mixed-integer formulation, while the associated fixed-snapshot abstract decision problem is NP-hard and the computing time increases for larger constellations.}
