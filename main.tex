...
\begin{itemize}
    ...
    \item \blue{\textbf{MRate:} Maximum link rate matching sets the weights of LCT pairs as their maximum transmission rates, $r_{n,m}$, so it emphasizes link-capacity-oriented topology design \cite{guo2024constellation}. Its traffic flows are then routed by open shortest path first (OSPF) \cite{moy1998ospf} using the link weights that are the reciprocal of aggregate satellite-pair rates $1/\sum_{\{n,m\}\in \mathcal{E}_{i,j}} c_{n,m}r_{n,m}$.}
