...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}l p{0.35\textwidth} l p{0.35\textwidth}@{}}
    ...
    $I$, $\mathcal{I}$ & Number of satellites; set of satellites & $\mathbf{l}_i(t)$, $\mathbf{v}_i(t)$ & Position and velocity of satellite $i$ in the Earth-centered inertial frame (m, m/s) \\
...
We construct the constellation in simulations based on the real-world Starlink constellation two-line element (TLE) data from CelesTrak \cite{CelesTrak} at $\mathcal{T}_0$ as UTC $2025-07-16$ $16:00$. To simulate different constellation sizes, we randomly select $I$ satellites out of the Starlink constellation.
...
Also, we observe that the network throughput reaches a near-maximum value around $K=500$ iterations. For the remaining simulations, we fix $\beta=0.5$ and $K=500$.
...
The dual-optimization iterations take about $1$ second each. However, the algorithm needs a few hundred iterations to converge, so the total computing time reaches the order of minutes for larger constellations and therefore still requires acceleration.
...
Similarly, the network throughput increases with the FOR because a larger FOR creates more connectable LCT pairs. The performance of the grid-based scheme varies little with the FOR, because only LCTs near the center of the steering range tend to be connected.
...
\begin{proof}
We use two instances throughout the proof. The source instance is denoted by $\mathsf{I}_{\mathrm{src}}$. It is an instance of the integral 2-commodity flow problem in symmetric digraphs. The constructed instance is denoted by $\mathsf{I}_{\mathrm{tar}}$. It is an instance of the decision problem AS-JMR. We organize the proof as explicit claims.
