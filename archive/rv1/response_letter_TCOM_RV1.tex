...
\item \textbf{Theory and algorithmic clarification.} We introduced the fixed-snapshot abstract snapshot joint matching and routing (AS-JMR) decision problem, stated the NP-hardness result as Proposition~\ref{prop:np_hardness}, and added a step-by-step appendix proof. We also clarified the scope of the hardness statement and revised the convergence discussion to emphasize the best-so-far dual guarantee and possible transient subgradient oscillations in the proposed algorithm.

\item \textbf{Optical-link and LCT modeling realism.} We removed the i.i.d.\ pointing-error assumption, explicitly stated the per-snapshot scope of the formulation with respect to ATP transition overhead, and added an ATP-time sensitivity evaluation. We also formalized the LCT visibility condition using the distance and mutual field-of-regard constraints, and clarified that inter-plane LISLs are feasible whenever the same visibility condition is satisfied.

\item \textbf{Evaluation strengthened.} We expanded the baseline set to include DRL, SaTE, MRate, and +Grid, while retaining random matching only as supporting context. We added a structured shell-based Starlink evaluation, a load-stress evaluation over higher offered demand, and structured comparisons across additional constellation settings. The previous random-subset evaluation is retained only as a separate scale study.

\item \textbf{Traffic, capacity, and operation scope clarified.} We narrowed the traffic-model claim to a snapshot-level spatial traffic profile, acknowledged omitted temporal and purchasing-capability effects, and clarified the definition of the gateway service capacity. We also stated that the optimization is intended as a centrally orchestrated per-snapshot planner carried out by the ground or network-control segment.

\item \textbf{Presentation and writing improved.} We revised figure captions and discussion text to make the comparisons and reported metrics clearer, and performed a focused proofreading pass across the introduction, notation, LCT-mechanics exposition, simulation discussion, complexity discussion, and conclusion.
\end{itemize}
