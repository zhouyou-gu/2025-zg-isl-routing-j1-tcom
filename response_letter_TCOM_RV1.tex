...
\item \textbf{Step-by-step NP-hardness proof.} The fixed-snapshot abstract decision problem, abstract snapshot joint matching and routing (AS-JMR), was introduced, Proposition~\ref{prop:np_hardness} now states the fixed-input scope of the NP-hardness claim directly, and a new appendix proves it through a step-by-step reduction from integral 2-commodity flow in symmetric digraphs.
...
We state Proposition \ref{prop:np_hardness} for fixed-snapshot AS-JMR, rather than directly for the full physical formulation \eqref{eq:prob:constrainted_routing_and_matching:primal}, because the reduction fixes the combinatorial snapshot input instead of deriving it from the geometry, field of regard constraints, distance thresholds, and traffic-pair construction rules. We therefore revised the proposition itself so that this fixed-input scope is stated in the proposition, and the following manuscript text no longer reads as a response-letter caveat.
...
``\dots\blue{This fixed-snapshot hardness result shows that the difficulty of joint link matching and flow routing is already present in the combinatorial coupling among matching, routing, and rate allocation once the snapshot connectability, link rates, and traffic-pair inputs are fixed. This motivates us to design efficient algorithms based on Lagrangian dual relaxation to solve the problem as follows.}''
