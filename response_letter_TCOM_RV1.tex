...
\textbf{Response:} In the revised manuscript, we distinguished the two Starlink simulation settings more clearly. The varying-$I$ and computing-time study used randomly sampled Starlink subsets, while the structured-shell study used one $I=1000$ satellite block from the dominant $53.2^\circ$ shell over six snapshot times. This kept the repeated multi-method, multi-snapshot study computationally manageable while preserving structured orbital geometry. The manuscript also included structured-constellation comparisons for a regular Walker-delta case and the OneWeb constellation in Fig. \ref{fig:plot_test_dual_optimization_different_constellation}. Random Starlink subsets were therefore used only for the varying-$I$ and computing-time study.
...
For direct inspection, we also reproduce the revised figure below.
...
For direct inspection, we also reproduce the updated comparison figures below.
