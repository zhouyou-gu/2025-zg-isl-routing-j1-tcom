...
\textbf{Response:} We agree that the original manuscript needed to make several modeling assumptions, the complexity discussion, and the simulation design more explicit. In the revised manuscript, we therefore clarified the scope of the pointing-jitter model and the ATP-free per-snapshot formulation, added a step-by-step NP-hardness treatment for the induced fixed-snapshot abstract decision problem, stated the non-monotone nature of the subgradient iterations more carefully, distinguished the random-subset and structured-shell Starlink evaluations, strengthened the baseline comparison, and performed a focused writing cleanup. Below, we respond point by point to each comment and identify the corresponding manuscript changes.
